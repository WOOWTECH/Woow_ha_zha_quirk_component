# WOOW ZHA Quirks + Tuya DP Sender

集中管理自訂 ZHA Quirks 與 Tuya DP 發送功能的 Home Assistant 套件。

透過 HACS 一鍵安裝，自動載入所有自訂 Zigbee 裝置 quirks，不需要手動設定 `custom_quirks_path`。

## 包含的 Integration

### 1. WOOW ZHA Quirks (`woow_zha_quirks`)

自動將內建的 quirk 檔案注入 ZHA，支援以下裝置：

| 裝置 | Manufacturer ID | 說明 |
|------|----------------|------|
| Simon i7 S2100 | `_TZ2000_sayvzx8w`, `_TZ2000_vvxwtxzf` | 智慧開關 |
| Tuya TS0001 | `_TZ3000_tqlv4ug4` | 單路開關 |
| Tuya TS0002 | `_TZ3000_denobasq` | 雙路開關 |
| Tuya TS0601 | `_TZE284_qxjkdfyt` | 捲簾電機 |
| Tuya TS0601 | `_TZE200_hmgktzj2` | 吊扇+燈 6速3模式 |
| Tuya TS0601 | `_TZE284_gt5al3bl` | Gledopto SPI LED 幻彩燈控制器 |
| Tuya TS0601 | `_TZE204_wwaeqnrf` | Zemismart 4路觸控開關 |
| Tuya TS0601 | `_TZE208_7aovt83n` | VRV 多聯機空調控制器 6區域 |
| Tuya TS0601 | `_TZE200_nogaemzt` | 窗簾軌道電機 |

另外包含 `patch_zha_climate.py`，修補 ZHA Climate 平台的 fan_mode 和 HVAC 模式擴充。

### 2. Tuya DP Sender (`tuya_dp_sender`)

透過 ZHA 發送原生 Tuya Datapoint 到 Zigbee 裝置。

**服務：**
- `tuya_dp_sender.send_dp` — 發送任意 DP 指令（BOOL/VALUE/ENUM）
- `tuya_dp_sender.send_dp_string` — 發送字串 DP

**平台：**
- `button` — 窗簾電機限位按鈕
- `select` — 窗簾電機模式選擇
- `switch` — 電機方向開關、Simon i7 全開關
- `text` — 4路開關螢幕標籤
...
3. 複製 `custom_components/tuya_dp_sender/` 到你的 HA `config/custom_components/`
4. 在 `configuration.yaml` 加入上述設定
5. 重啟 Home Assistant
