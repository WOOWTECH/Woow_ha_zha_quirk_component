...
| 17 | Simon i7 17-70E857TY | TS1002 0-10V Smart Dimming Remote Switch (2-gang) | `_TZ3210_qe3d5gga` | `simon_i7_70e857ty_dimmer.py` | `binary_sensor` + `sensor` + `select` | 2 gang On/Off **binary_sensors** (Gang 1 / Gang 2) that **mirror** the physical wall-switch state — device is a remote whose server OnOff rejects on/off (`UNSUP_CLUSTER_COMMAND`), so the control-less default switches are suppressed and replaced with read-only binary_sensors; 2 per-gang Brightness **sensors** (%) fed by the slide-dim `current_level`, which reports on push; 2 per-gang Min Brightness **numbers** (%, 5-50) that set the floor of the slide travel via Tuya DP 103/104 (`send_data` 0x04, mirrored read-only on `min_level`); single device-global Status Light indicator mode (Close / Switch Status / Switch Position), Identify button + duplicate firmware/OTA entities removed, dead StartUpOnOff selects suppressed |
...
| 39 | Simon 241E8016TY | TS0052 2-Gang Dimmer | `_TZ3002_cqpubrcz` | `ts0052_dimmer_TZ3002_cqpubrcz.py` | `light` ×2 + `select` + `number` ×4 | Standard-ZCL 2-gang dimmer (both endpoints are real gangs, no Tuya `0xEF00`); on/off + brightness per gang + indicator LED mode (OnOff 0x8001). **Built to the `app_version` 132 standard** — this SKU ships in two firmware revisions that are identical in every identity field except Basic `app_version`, so `QuirkBuilder` cannot tell them apart and one registration has to serve both ([ADR 0003](docs/adr/0003-firmware-divergence-within-one-quirk-registration.md)). **Min/max brightness** are four writable `%` `number`s (one pair per gang) on standard MinLevel/MaxLevel (0x0002/0x0003): on **fw 132** the writes succeed and the firmware enforces the window on HA commands *and* on the physical wall button (sniffer- and hardware-proven, 16/16 writes SUCCESS); on **fw 129** the device answers `READ_ONLY` (0x88) to everyone including the Tuya gateway itself (27/27), and the entity surfaces that as a visible error rather than failing silently. Neither revision has a writable Tuya min/max attr or an `0xEF00` DP (unlike the TS110D sibling). The quirk refuses `min > max` itself, because the device stores an inverted window without complaint. **Indicator mode** is written as `uint8` (0x20) — fw 132 rejects the upstream `enum8` with `INVALID_DATA_TYPE`; this fixes fw 132 and knowingly breaks the same control on fw 129, which wants `enum8`. No single type serves both. **Upgrading from 1.3.0:** the two brightness limits per gang were `sensor`s and are `number`s in 1.3.1 — same unique_id, different platform, so the old `sensor.<dimmer>_{min,max}_brightness_{1,2}` rows linger in the entity registry as permanently unavailable alongside the new `number.*` ones. Delete the stale `sensor.*` rows (Settings → Devices & Services → Entities) and repoint any dashboard card or automation at the `number.*` ids. **Power-on behaviour/level removed** (StartUpOnOff 0x4003 / StartUpCurrentLevel 0x4000 are ACKed+stored but never applied at power-up, and the app has no power-on setting for this SKU — dead controls). Transition-time/on-level/move-rate + duplicate Tuya power-on/child-lock + per-endpoint firmware/OTA suppressed |
...
Firmware `app_version` 134 declares a `0xEF00` Tuya MCU cluster on both endpoints. Gang state and
slide-dim level do **not** go through it — taps and slides all arrive as standard ZCL
`Report_Attributes` on `0x0006` / `0x0008` — but it does carry the **per-gang minimum brightness**
(DP 103 / DP 104). That cluster only answers to the `send_data` (0x04) command; the zhaquirks
...
This is why earlier versions of this quirk shipped the two as read-only `sensor`s — every ZHA test that "proved" the attributes inert wrote them ~1.5 s apart. They are now writable `number`s again. (The TS0052 sibling is read-only **on firmware `app_version` 129** — it rejects every write with ZCL `READ_ONLY` 0x88. On `app_version` 132 the same writes succeed and are enforced, so that quirk also ships writable `number`s; see [ADR 0003](docs/adr/0003-firmware-divergence-within-one-quirk-registration.md).)
...
│           ├── ts0052_dimmer_TZ3002_cqpubrcz.py            # Simon 241E8016TY — TS0052 2-gang dimmer (built to fw 132)
