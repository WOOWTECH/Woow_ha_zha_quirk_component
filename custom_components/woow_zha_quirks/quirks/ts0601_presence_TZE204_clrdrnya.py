"""ZHA Quirk for Tuya mmWave human-presence sensor _TZE204_clrdrnya (WO_40117).

Tuya product "人体存在传感器" (human-presence sensor), category ``hps``, product_id
``clrdrnya``, model **MTG235-ZB-RL**. A 24 GHz mmWave radar occupancy sensor with a built-in
relay output (通斷器) that can auto-switch a load on presence. TS0601 / 0xEF00 Tuya-MCU device.

ZHA signature (read from the paired device, IEEE 44:9f:da:ff:fe:62:48:bf):
  manufacturer = "_TZE204_clrdrnya", model = "TS0601"
...
This mapping mirrors the proven upstream ``zhaquirks.tuya.tuya_motion`` ``_TZE204_clrdrnya``
builder (so behaviour/labels are unchanged) but is owned by the woow component and additionally:
...
displayed value = raw / 10^scale; see tuya_export/DP_REFERENCE.md → WO_40117):
  ...
  DP102 - value - fading_time (離場延時) 0-600 s         -> number  "Fading time"
  DP103 - string- cli ("cline") opaque                  -> not exposed
  DP104 - value - illuminance (sc 1)                    -> illuminance sensor (0x0400, ÷10)
  DP105 - value - trigger_sensitivity 0-9               -> number  "Entry sensitivity"
  DP106 - value - trigger_distance 0-9 m (sc 2)         -> number  "Entry distance indentation"
  ...
  DP112 - value - block_time 0-10 s (sc 1)              -> number  "Block time"
...
class TuyaBreakerMode(t.enum8):
    ...
    The Tuya thing-model declares [standard, local, force, none], but this firmware only
    IMPLEMENTS standard/local — writing force(2)/none(3) is rejected by the MCU and HA reverts
    (verified live), so they are intentionally omitted (matches upstream zhaquirks).
    """
...
class TuyaBreakerPolarity(t.enum8):
    ...
    The thing-model declares [close(NC), open(NO)], but this unit's relay is FIXED as NO —
    writing NC(0) is rejected (stays NO) and the device reports parameter_result = PolarityError
    (verified live). So NC is omitted; the select shows only NO (the actual, unchangeable polarity).
    """
...
(
    TuyaQuirkBuilder("_TZE204_clrdrnya", "TS0601")
    ...
    # but the FIRMWARE floor is 0.3 m (0/0.1/0.2 are rejected — verified live), so min_value=0.3.
    ...
    # (upstream's 0.75 min/step was for a different variant; 0.75 was rejected -> HA reverted.)
    .tuya_number(
        dp_id=4,
    ...
    # Thing-model min is 1 (0.1 s) but the FIRMWARE floor is 1.5 s (0.1..1.2 s are rejected —
    # verified live), so min_value=1.5.
    .tuya_number(
        dp_id=112,
    ...
    # presence and overrides manual writes (setting On is rejected → reverts). Verified live.
    .tuya_enum(
        dp_id=108,
